...
class AuthRepository:
    def __init__(self):
        self.jwt_handler = JWTHandler(auth_secret_key, auth_algorithm)
    ...
    def register_user(self, user: UserCreateSchema) -> dict:
        ...
        hashed_password = self.jwt_handler.hash_password(user.password)
        new_user = user_repo.create_user(UserCreateSchema(
            name=user.name,
            email=user.email,
